...
async def test_kb_similarity_edge_cases_sorting_and_threshold():
    ...
    p1 = LearnedPattern("p1", "pattern", {"k": 1}, {}, 0.9, datetime.now(timezone.utc))
    p2 = LearnedPattern("p2", "pattern", {"k": 2}, {}, 0.5, datetime.now(timezone.utc))
    p3 = LearnedPattern("p3", "pattern", {"k": 1, "x": 3}, {}, 0.7, datetime.now(timezone.utc))
...
async def test_telemetry_handler_can_handle_and_filter():
    ...
    state = SystemState("sysZ", datetime.now(timezone.utc), {"cpu": MetricValue("cpu", 1.0)}, HealthStatus.HEALTHY)
    ...
    state2 = SystemState("sysQ", datetime.now(timezone.utc), {"cpu": MetricValue("cpu", 2.0)}, HealthStatus.HEALTHY)
