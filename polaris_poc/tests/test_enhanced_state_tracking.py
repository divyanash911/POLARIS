...
dotenv.load_dotenv(Path("/home/prakhar/dev/prakhar479/POLARIS/polaris_poc/.env"))
...
class TestEnhancedStateTracking:
    ...
    async def process_test_events(self, world_model: GeminiWorldModel, events: List[Tuple[str, float]]) -> None:
    ...
    async def test_enhanced_telemetry_processing(self, world_model):
        ...
        assert len(world_model._current_system_state) > 0, "Current state should be populated"
        assert len(world_model._system_state_history) > 0, "State history should be populated"
        assert len(world_model._state_embeddings) > 0, "State embeddings should be generated"
        assert len(world_model._temporal_state_index) > 0, "Temporal index should be populated"
        ...
        for metric_name, state_entry in world_model._current_system_state.items():
            ...
            assert "anomaly_score" in metadata, f"Anomaly score missing for {metric_name}"
            assert "trend" in metadata, f"Trend missing for {metric_name}"
            
            # Verify confidence is in valid range
            assert 0.0 <= metadata["confidence"] <= 1.0, f"Invalid confidence for {metric_name}"
            assert 0.0 <= metadata["anomaly_score"] <= 1.0, f"Invalid anomaly score for {metric_name}"
    ...
    async def test_state_consistency_validation(self, world_model):
        ...
        assert len(world_model._state_consistency_log) > 0, "Consistency checks should be logged"
        
        # Check for detected issues
        consistency_issues = [
            check for check in world_model._state_consistency_log 
    ...
    async def test_vector_embedding_generation(self, world_model):
        ...
        for embedding_key, embedding_data in world_model._state_embeddings.items():
            assert "embedding" in embedding_data, f"Embedding vector missing for {embedding_key}"
            assert "metric_name" in embedding_data, f"Metric name missing for {embedding_key}"
            assert "timestamp" in embedding_data, f"Timestamp missing for {embedding_key}"
            assert "metadata" in embedding_data, f"Metadata missing for {embedding_key}"
            
            # Verify embedding is a list of floats
            embedding = embedding_data["embedding"]
            assert isinstance(embedding, list), f"Embedding should be a list for {embedding_key}"
            assert len(embedding) == 8, f"Embedding should have fixed size for {embedding_key}"
            assert all(isinstance(x, float) for x in embedding), f"Embedding should contain floats for {embedding_key}"
    ...
    async def test_temporal_indexing(self, world_model):
        ...
        for index_key, states in world_model._temporal_state_index.items():
            assert isinstance(states, list), f"Index should contain list for {index_key}"
            for state in states:
                assert "value" in state, f"State should have value in index {index_key}"
                assert "timestamp" in state, f"State should have timestamp in index {index_key}"
    ...
    async def test_llm_state_evolution_analysis(self, world_model):
        ...
        assert len(world_model._state_evolution_patterns) > 0, "Evolution patterns should be tracked"
        
        # Check for CPU usage evolution pattern
        cpu_pattern_key = "cpu.usage_evolution"
        assert cpu_pattern_key in world_model._state_evolution_patterns, "CPU evolution pattern should exist"
    ...
    async def test_enhanced_health_status(self, world_model):
        ...
        assert "configuration" in health_status, "Health status should have configuration"
        assert "state_tracking_status" in health_status, "Health status should have state tracking status"
        ...
        for metric in enhanced_metrics:
            ...
            assert isinstance(metrics[metric], int), f"Enhanced metric {metric} should be integer"
        ...
        for config_flag in enhanced_config:
            assert config_flag in config, f"Enhanced config {config_flag} should be present"
            assert isinstance(config[config_flag], bool), f"Enhanced config {config_flag} should be boolean"
        ...
        for field in status_fields:
            assert field in tracking_status, f"State tracking status {field} should be present"
            assert isinstance(tracking_status[field], str), f"State tracking status {field} should be string"
    ...
    async def test_confidence_scoring(self, world_model):
        ...
        assert confidence > 0.8, f"High confidence expected for complete data, got {confidence}"
...
async def run_integration_test():
    ...
    try:
        ...
        health_status = await world_model.get_health_status()
        print(f"\n📊 Final Test Metrics:")
        print(f"   • State metrics tracked: {health_status['metrics']['current_state_metrics']}")
        print(f"   • Embeddings generated: {health_status['metrics']['state_embeddings_stored']}")
        print(f"   • Temporal index keys: {health_status['metrics']['temporal_index_keys']}")
        print(f"   • Evolution patterns: {health_status['metrics']['evolution_patterns_tracked']}")
        print(f"   • Consistency checks: {len(world_model._state_consistency_log)}")
        print(f"   • Anomalies detected: {health_status['metrics']['anomalies_detected']}")
