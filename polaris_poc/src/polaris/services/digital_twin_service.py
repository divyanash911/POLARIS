...
from polaris.models.world_model import (
    WorldModel, QueryRequest, QueryResponse,
        ...
        try:
            self.logger.debug(
                ...
                extra={
                    ...
                    "query_content": request.query_content[:100] + "..." if len(request.query_content) > 100 else request.query_content
                }
            )
        ...
        try:
            self.logger.debug(
                ...
                extra={
                    ...
                    "anomaly_description": request.anomaly_description[:100] + "..." if len(request.anomaly_description) > 100 else request.anomaly_description
                }
            )
        ...
        try:
            ...
            if request.operation == "health_check":
                ...
                health_status = digital_twin_pb2.HealthStatus(
                    ...
                    last_check=health_data.get("last_check", datetime.now(timezone.utc).isoformat()),
                ...
                service_metrics.update({
                    ...
                    "uptime_sec": (datetime.now(timezone.utc) - datetime.fromisoformat(self._metrics["service_start_time"].replace('Z', '+00:00'))).total_seconds()
                })
